...
async def Channellockcommand(
    ...
    if locktime == "resolveall" and unlocktime == "!@#$%^&*()":
        ...
        await interaction.send("Resolve Successful!", ephemeral=True)

    overwrite = channelinput.overwrites_for(interaction.guild.default_role)
    if not overwrite.send_messages and not overwrite.send_messages_in_threads:
        await interaction.send(
            f"Sorry {interaction.user.mention},"
            " this channel has already been locked. please unlock and try again.",
            ephemeral=True,
        )
        return
    ...
    if locktime < 0 or unlocktime < 0:
        await interaction.send(
            f"{'L' if locktime < 0 else 'Unl'}ocktime is invalid. please try with values greater than 0.",
            ephemeral=True,
        )
        return
        ...
        await interaction.send(
            "the unlock time has to be after lock time. please try again.",
            ephemeral=True,
        )
        return
        ...
        await interaction.send(
            f"{'L' if locktime < timenow else 'Unl'}ocktime has already passed. the current time is {round(time.time())}. please try again.",
            ephemeral=True,
        )
        return
    ...
    await interaction.send(
        f"{channel_id} is scheduled to lock on {unixlocktime} and unlock on {unixunlocktime}",
        ephemeral=True,
    )
    ...
    await channelinput.send(
        f"This channel has been scheduled to lock <t:{max(locktime, timenow)}:R>."
    )
...
async def Forumlockcommand(
    ...
    threadinput: discord.Thread = discord.SlashOption(
        name="thread_name",
        description="Which thread do you want to lock?",
        required=True,
    ),
    locktime: str = discord.SlashOption(
        name="lock_time",
        description="At what time do you want the thread to be locked?",
        required=True,
    ),
    unlocktime: str = discord.SlashOption(
        name="unlock_time",
        description="At what time do you want the thread to be unlocked?",
        required=True,
    ),
):
    timenow = int(time.time()) + 1
    ...
    if threadinput.locked:
        await interaction.send(
            f"Sorry {interaction.user.mention},"
            " the thread has already been locked. please unlock and try again.",
            ephemeral=True,
        )
        return
    ...
    await interaction.send(
        f"{thread_id} is been scheduled to lock on {unixlocktime} and unlock on {unixunlocktime}",
        ephemeral=True,
    )
    ...
    await threadinput.send(
        f"this thread has been scheduled to lock <t:{max(locktime, timenow)}:R> successfully."
    )
