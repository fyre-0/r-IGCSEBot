...
async def save_questions(questions: list, session_id: str) -> list[Question]:
    questions_mapped = list(map(lambda x: Question(
        question_name=f"{x['subject']}_{x['season']}{str(x['year'])[2:4]}_qp_{x['paper']}{x['variant']}_q{x['questionNumber']}", questions=x["questions"], answers=x["answers"], session_id=session_id), questions))
...
async def list_sessions(interaction: discord.Interaction):
    ...
    embed = discord.Embed(title="Public sessions", color=discord.Color.random(
    ), description="Use `/practice join <session_id>` to join a session!")
    for session in sessions:
        embed.add_field(name=f"Session ID: {session.session_id}",
...
async def add_to_session(interaction: discord.Interaction):
    ...
    user_owns_session = Session.find(
        Session.started_by == str(interaction.user.id)).all()
...
async def session_info(interaction: discord.Interaction):
    ...
    sessions = Session.find(
        Session.private == 0 or Session.started_by == interaction.user.id).all()
    ...
    embed.add_field(
        name="Subject", value=practice_subjects[session["subject"]])
    embed.add_field(name="Users", value=', '.join(
        list(map(lambda x: f"<@{x}>", session["users"]))))
