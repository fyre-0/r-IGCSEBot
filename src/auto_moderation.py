...
async def on_auto_moderation_action_execution(automod_execution):
    ...
    if automod_execution.action.type.name == "timeout":
        ...
        ban_message_channel = bot.get_channel(gpdb.get_pref(
            "modlog_channel", automod_execution.guild_id))

        if ban_message_channel:
            try:
                last_ban_message = await ban_message_channel.history(limit=1).flatten()
                case_no = (int("".join(
                    list(filter(str.isdigit, last_ban_message[0].content.splitlines()[0])))) + 1)
            except:
                case_no = 1
            ...
            timeout_duration_simple = convert_time((str(timeout_time_seconds // 86400), str(
                (timeout_time_seconds % 86400) // 3600), str((timeout_time_seconds % 3600) // 60), str(timeout_time_seconds % 60)))
            punishdb.add_punishment(
                case_no, member.id, "Automod", reason, "Timeout", duration=timeout_duration_simple)
