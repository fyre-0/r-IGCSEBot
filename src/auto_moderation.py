...
async def on_auto_moderation_action_execution(automod_execution):
    ...
    if automod_execution.action.type.name == "timeout":
        ...
        timeout_time_seconds = (
            automod_execution.action.metadata.duration_seconds
        ...
        if ban_message_channel:
                    ...
                    int(
                        "".join(
                            list(
                                filter(
                                    str.isdigit,
                                    last_ban_message[0].content.splitlines()[0],
                                )
                            )
                        )
                    )
            ...
            timeout_duration_simple = convert_time(
                (
                    str(timeout_time_seconds // 86400),
                    str((timeout_time_seconds % 86400) // 3600),
                    str((timeout_time_seconds % 3600) // 60),
