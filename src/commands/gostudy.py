...
async def gostudy(
    ...
    mute_time: Optional[str] = discord.SlashOption(
        name="time",
        description="how long should the mute last for? (default: 1 hour)",
        required=False,
    ),
    user: Optional[discord.User] = discord.SlashOption(
        name="name",
        description="who do you want to use this command on? (for mods)",
        required=False,
    ),
):
    ...
    if mute_time:
        ...
        if time_to_mute < 600:
            await interaction.send(
                "The minimum time for gostudy is 10 minutes!", ephemeral=True
            )
            return
    if user is None:
        ...
        if user_already_fmuted:
            if int(user_already_fmuted["unmute_time"]) >= (
                (int(time.time()) + 1) + time_to_mute
            ):
                await interaction.send(
                    "You're already muted, and you cannot reduce the time of your mute. Go Study!",
                    ephemeral=True,
                )
                return
        view = discord.ui.View(timeout=None)
        proceedBTN = discord.ui.Button(
            label="Proceed", style=discord.ButtonStyle.blurple
        )
        ...
        if user_already_fmuted:
            if int(user_already_fmuted["unmute_time"]) >= (
                ...
                await interaction.send(
                    "This user is already muted and you cannot reduce the time of their mute. LET THEM STUDY!",
                    ephemeral=True,
                )
                return
        view = discord.ui.View(timeout=None)
...
async def remove_gostudy(
    ...
    user: discord.User = discord.SlashOption(
        name="name",
        description="who do you want to use this command on?",
        required=False,
    ),
):
    ...
    Logging = guild.get_channel(MODLOG_CHANNEL_ID)
    if user is None:
        user_id = interaction.user.id
        ...
        embed = discord.Embed(
            description="Go Study Mode Deactivated", colour=discord.Colour.green()
        )
