...
async def advstick(
    ...
    message_id: str = discord.SlashOption(
        name="messageid", description="ID of the message you want to stick in the future (must be in the current channel)", required=True
    )
):
    ...
    await smdb.timed_sticky(channel, await interaction.channel.fetch_message(message_id), stick_time, unstick_time)
