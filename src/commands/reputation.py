...
async def rep(
    ...
    user: discord.User = discord.SlashOption(
        name="user", description="User to view rep of", required=False
    ),
):
...
async def change_rep(
    interaction: discord.Interaction,
    user: discord.User = discord.SlashOption(
        name="user", description="User to view rep of", required=True
    ),
    new_rep: int = discord.SlashOption(
        name="new_rep",
        description="New rep amount",
        required=True,
        min_value=0,
        max_value=99999,
    ),
...
async def leaderboard(
    ...
    page: int = discord.SlashOption(
        name="page",
        description="Page number to to display",
        required=False,
        min_value=1,
        max_value=99999,
    ),
    ...
    for n, chunk in enumerate(chunks):
        embed = discord.Embed(
            ...
            description=f"Page {n + 1} of {len(chunks)}",
    ...
    async def f_callback(b_interaction):
        if b_interaction.user != interaction.user:
            await b_interaction.response.send_message(
                "This is not for you.", ephemeral=True
            )
            return
        nonlocal page
        view = discord.ui.View(timeout=None)
        first.disabled, prev.disabled, nex.disabled, last.disabled = (
            True,
            True,
            False,
            False,
    ...
    first.callback, prev.callback, nex.callback, last.callback = (
        f_callback,
        p_callback,
        n_callback,
        l_callback,
