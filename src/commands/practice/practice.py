...
async def close_session(session: Session, message: str):
    ...
    if len(solved_questions) != len(questions):
        unsolved_message = (
            f" out of which only {len(solved_questions)} were solved by everyone"
        )
    ...
    if len(number_of_correct_answers) > 25:
        for i in range(len(number_of_correct_answers) // 25):
            for user in list(number_of_correct_answers.keys())[i * 25 : (i + 1) * 25]:
                if len(embeds) != 0:
                    embed = discord.Embed()
                member = thread.guild.get_member(
                    int(user)
                ) or await thread.guild.fetch_member(int(user))
                embed.add_field(
                    name=f"{member.name}",
                    value=f"{number_of_correct_answers[user]}/{number_of_answers[user]}",
                )
            embeds.append(embed)
    else:
...
async def new_session(interaction: discord.Interaction):
    ...
    if user_in_session and user_in_session["playing"]:
        await interaction.response.send_message(
            "You are already in a session! Please leave that session using `/practice leave`.",
            ephemeral=True,
        )
        return
    ...
    if not questions or len(list(questions)) == 0:
        await msg.edit(
            content="No questions found for the selected subject and topics.", view=None
        )
        TempSessionData.delete(str(interaction.user.id))
        return
    ...
    await msg.edit(
        content=f"Created a new practice session in {thread.mention}!", view=None
    )
    ...
    await thread.send(
        f"{interaction.user.mention} has started a new practice session!\n\nUsers: {', '.join(mentioned_users)}\nSession ID: {session.session_id}"
    )
    session.save()
...
async def leave_session(interaction: discord.Interaction):
    ...
    if not user or not user["playing"]:
        await interaction.response.send_message(
            "You are currently not in a session!", ephemeral=True
        )
        return
    ...
    if session["started_by"] == str(interaction.user.id):
        await interaction.response.send_message(
            "You cannot leave a session you started! Please end the session using `/practice end` instead.",
            ephemeral=True,
        )
        return
...
async def end_session(interaction: discord.Interaction):
    ...
    if session["started_by"] != str(interaction.user.id):
        await interaction.response.send_message(
            "You cannot end a session you did not start! Use `/practice leave` to leave the session instead.",
            ephemeral=True,
        )
        return
...
async def list_sessions(interaction: discord.Interaction):
    ...
    if not sessions:
        await interaction.response.send_message(
            "There are no public sessions available at the moment! Create one by running `/practice new`.",
            ephemeral=True,
        )
        return
    ...
    embed = discord.Embed(
        ...
        description="Use `/practice join <session_id>` to join a session!",
    )
    for session in sessions:
        embed.add_field(
            name=f"Session ID: {session.session_id}",
            value=f"Subject: {practice_subjects[session['subject']]}\nUsers: {len(session['users'])}",
            inline=False,
        )
...
async def pause_session(interaction: discord.Interaction):
    ...
    if session["started_by"] != str(interaction.user.id):
        await interaction.response.send_message(
            "You cannot pause a session you did not start!", ephemeral=True
        )
        return

    if session["paused"] == 1:
        await interaction.response.send_message(
            "The session is already paused!", ephemeral=True
        )
        return
...
async def resume_session(interaction: discord.Interaction):
    ...
    if session["started_by"] != str(interaction.user.id):
        await interaction.response.send_message(
            "You cannot resume a session you did not start!", ephemeral=True
        )
        return

    if session["paused"] == 0:
        await interaction.response.send_message(
            "This session is not paused!", ephemeral=True
        )
        return
...
async def add_to_session(interaction: discord.Interaction):
    ...
    user_owns_session = Session.find(
        Session.started_by == str(interaction.user.id)
    ).all()
    if not user_owns_session:
        await interaction.response.send_message(
            "Only the owner of the session can add others!", ephemeral=True
        )
        return
    ...
    interaction.message = await interaction.send(
        "Select a user to add to the session!", view=userview, ephemeral=True
    )
    ...
    if user_in_another_session and user_in_another_session["playing"]:
        await interaction.edit(
            "That user is already in another session!", ephemeral=True
        )
        return
...
async def remove_from_session(interaction: discord.Interaction):
    ...
    if not user_owns_session:
        await interaction.response.send_message(
            "Only the owner of the session can remove others!", ephemeral=True
        )
        return
    ...
    userview = AddRemoveUserView()
    interaction.message = await interaction.send(
        "Select a user to remove from the session!", view=userview, ephemeral=True
    )
...
async def session_info(interaction: discord.Interaction):
    ...
    sessions = Session.find(
        Session.private == 0 or Session.started_by == interaction.user.id
    ).all()

    if not sessions:
        await interaction.response.send_message(
            "There are no public sessions available at the moment!", ephemeral=True
        )
        return
    ...
    interaction.message = await interaction.send(
        "Select a session to see more information about!", view=view, ephemeral=True
    )
    ...
    embed = discord.Embed(
        title=f"Session ID: {session.session_id}", color=discord.Color.random()
    )
    embed.add_field(name="Subject", value=practice_subjects[session["subject"]])
    embed.add_field(
        name="Users", value=", ".join(list(map(lambda x: f"<@{x}>", session["users"])))
    )
...
async def join_session(interaction: discord.Interaction):
    ...
    view = JoinSessionListView(list_of_sessions, interaction)
    interaction.message = await interaction.send(
        "Select a session to join!", view=view, ephemeral=True
    )
...
async def practice(
    ...
    action: str = discord.SlashOption(
        name="action",
