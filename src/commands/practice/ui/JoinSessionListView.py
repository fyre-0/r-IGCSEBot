...
class JoinSessionListView(discord.ui.View):
    def __init__(
        self, sessions: list[discord.SelectOption], interaction: discord.Interaction
    ):
        super().__init__(timeout=300)
        select_menu = discord.ui.Select(
            custom_id="join_session", placeholder="Select a session", options=sessions
        )
        self.add_item(select_menu)

        self.join_button = discord.ui.Button(
            label="Join", style=discord.ButtonStyle.success, custom_id="join"
        )
