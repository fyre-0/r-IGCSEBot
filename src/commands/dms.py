...
async def delete_dm_thread(ctx, member: discord.Member = None):
    ...
    if not member:
        if not await is_moderator(ctx.author) and not await is_chat_moderator(
            ctx.author
        ):
            await ctx.send(
                f"You are not permitted to use this command, {member.mention}!"
            )
            return
    else:
    ...
    if not thread:
        ...
        await ctx.send(
            "DM thread deleted! If DMs are still closed, a new thread will be made."
        )
