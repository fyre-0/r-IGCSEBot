...
async def cheminfo(
    ...
    try:
        compound = pcp.get_compounds(name or formula, "formula" if formula else "name")[
            0
        ]
        ...
        synonyms = (
            list(
                filter(
                    lambda x: not re.search(r"[0-9]", x)
                    and x != compound.molecular_formula,
                    compound.synonyms[:10],
                )
            )
            or None
        )
        ...
        embed = discord.Embed(
            title=f"{synonyms[0] if synonyms else compound.iupac_name} ({formula_label})",
            color=0x66F18E,
        )
        embed.description = f"For more information, [click here](https://pubchem.ncbi.nlm.nih.gov/compound/{compound.cid})"
        embed.add_field(
            name="Molecular Weight",
            value=f"{round(float(compound.molecular_weight))} g/mol",
            inline=True,
        )
        if is_element:
            embed.add_field(
                name="Atomic Number",
                value=f"{round(compound.atoms[0].number)}",
                inline=True,
            )

        if experimental_properties["description"]:
            embed.add_field(
                name=f"Visual description of {'element' if is_element else 'compound'}",
                value=experimental_properties["description"],
                inline=False,
            )

        if synonyms:
            embed.add_field(
                name="Synonyms", value=", ".join(synonyms[:5]), inline=False
            )
        ...
        await interaction.send(
            "Invalid formula or name (an error occurred)", ephemeral=True
        )
    ...
    try:
        ...
        for property in experimental_properties["Section"]:
            if property["TOCHeading"] == "Physical Description":
                description_of_compound = property["Information"][0]["Value"][
                    "StringWithMarkup"
                ][0]["String"]
                color = property["Information"][1]["Value"]["StringWithMarkup"][0][
                    "String"
                ]
                if color:
                    color = color.split(";")[0]

        return {"description": description_of_compound, "color": color}
    except Exception:
