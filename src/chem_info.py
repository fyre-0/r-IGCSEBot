...
metals = ["Li", "Be", "Na", "Mg", "K", "Ca", "Rb", "Sr", "Cs", "Ba", "Fr", "Ra", "Sc", "Y", "Ti", "Zr", "Hf", "V", "Nb", "Ta", "Cr", "Mo", "W", "Mn",
          "Tc", "Re", "Fe", "Ru", "Os", "Co", "Rh", "Ir", "Ni", "Pd", "Pt", "Cu", "Ag", "Au", "Zn", "Cd", "Hg", "Al", "Ga", "In", "Sn", "Tl", "Pb", "Bi"]
...
async def cheminfo(interaction: discord.Interaction,
    ...
    try:
        compound = pcp.get_compounds(
            name or formula, 'formula' if formula else 'name')[0]
        ...
        synonyms = list(filter(lambda x: not re.search(
            r"[0-9]", x) and x != compound.molecular_formula, compound.synonyms[:10])) or None
        ...
        embed = discord.Embed(
            title=f"{synonyms[0] if synonyms else compound.iupac_name} ({formula_label})", color=0x66F18E)
        embed.description = f"For more information, [click here](https://pubchem.ncbi.nlm.nih.gov/compound/{compound.cid})"
        embed.add_field(name="Molecular Weight",
                        value=f"{round(float(compound.molecular_weight))} g/mol", inline=True)
        if is_element:
            embed.add_field(
                name="Atomic Number", value=f"{round(compound.atoms[0].number)}", inline=True)

        if experimental_properties["description"]:
            embed.add_field(name=f"Visual description of {'element' if is_element else 'compound'}",
                            value=experimental_properties["description"], inline=False)

        if synonyms:
            embed.add_field(name="Synonyms", value=', '.join(
                synonyms[:5]), inline=False)

        if experimental_properties["color"]:
            embed.add_field(
                name="Color", value=experimental_properties["color"], inline=True)
        ...
        if is_ion:
            embed.add_field(
                name="Charge", value=f"{compound.charge}", inline=True)
