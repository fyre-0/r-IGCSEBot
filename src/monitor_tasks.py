...
async def toggleforumlock(thread_id, unlock, unlocktime):
    ...
    try:
        ...
        if unlock:
            ...
            embed.set_author(name="MongoDB#0082", icon_url="https://cdn.discordapp.com/attachments/947859228649992213/1196753678342819933/mongodb.png?ex=65b8c6b7&is=65a651b7&hm=db7fdb12435ba54299497dfb26f65dac5993caa8b48b976cf01238233c54a508&")
            embed.add_field(
                name="Channel", value=f"<#{thread_id}>", inline=False)
            embed.add_field(name="Date", value=f"<t:{timern}:F>", inline=False)
            embed.add_field(
...
async def autorefreshhelpers():
    ...
    for chnl, role in helper_roles.items():
        try:
            helper_role = discord.utils.get(
                bot.get_guild(GUILD_ID).roles, id=role)
    ...
    if changed:
        ...
        embed.add_field(name="Channels",
                        value="no changes were made.", inline=False)
        embed.add_field(name="Date", value=f"<t:{timenow}:F>", inline=False)
        embed.add_field(
            name="ID", value=f"```py\nBot = {bot.user.id}\nChannel = 697072778553065542```", inline=False)
        embed.set_footer(text=f"{bot.user}",
    ...
    try:
        ...
        for result in results:
            if result["time"] <= time.time():
                ...
                clocks.update_one({"_id": result["_id"]}, {
                                  "$set": {"resolved": True}})
        ...
        for result in results:
            if result["time"] <= time.time():
                ...
                flocks.update_one({"_id": result["_id"]}, {
                                  "$set": {"resolved": True}})
    ...
    for result in results:
        try:
            if int(result["unmute_time"]) <= timern:
                ...
                embed.set_author(name="MongoDB#0082", icon_url="https://cdn.discordapp.com/attachments/947859228649992213/1196753678342819933/mongodb.png?ex=65b8c6b7&is=65a651b7&hm=db7fdb12435ba54299497dfb26f65dac5993caa8b48b976cf01238233c54a508&")
                embed.add_field(
                    name="User", value=f"{user.mention}", inline=False)
                embed.add_field(
                    name="Date", value=f"<t:{timern}:F>", inline=False)
                embed.add_field(
...
async def send_questions():
    sessions = Session.find(
        Session.paused == 0 and Session.currently_solving == "none").all()
    for session in sessions:
        ...
        questions = Question.find(
            Question.session_id == session.session_id).all()
        questions: list[Question] = list(
            filter(lambda x: x["solved"] == 0, questions))
