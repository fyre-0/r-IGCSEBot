...
async def history(interaction: discord.Interaction, user: discord.User = discord.SlashOption(name="user", description="User to view history of", required=True)):
    ...
    for result in results:
        ...
        if type(result['when']) == datetime.datetime:
            ...
            date_of_event = datetime.datetime.fromisoformat(
                str(result['when'])).strftime("%d %b, %Y at %I:%M %p")
    ...
    if len(history) == 0:
        ...
        text += "\n".join(
            list(map(lambda x: f"{x[0]}: {x[1]}", list(actions.items()))))
        text += f"\n"
        text += f"\nFurther Details:\n"
        text += ('\n'.join(history))[:1900]
...
async def warn(interaction: discord.Interaction,
    ...
    if warnlog_channel:
        ...
        try:
            ...
            case_no = int(
                ''.join(list(filter(str.isdigit, last_ban_msg[0].content.splitlines()[0])))) + 1
        except:
            case_no = 1
    ...
    punishdb.add_punishment(
        case_no, user.id, interaction.user.id, reason, action_type)
...
Until: <t:{int(time.time()) + seconds}> (<t:{int(time.time()) + seconds}:R>)''')
    ...
    timeout_duration_simple = convert_time((str(seconds // 86400), str(
        (seconds % 86400) // 3600), str((seconds % 3600) // 60), str(seconds % 60)))
    punishdb.add_punishment(case_no, user.id, interaction.user.id,
                            reason, action_type, duration=timeout_duration_simple)
...
Moderator: {mod}"""
    ...
    punishdb.add_punishment(
        case_no, user.id, interaction.user.id, "", action_type)
