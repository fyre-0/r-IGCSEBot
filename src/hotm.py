...
async def votehotm(interaction: discord.Interaction,
                   helper: discord.Member = discord.SlashOption(name="helper", description="Choose the helper to vote for", required=True)):
    if helper.bot:
        ...
        if len(messages) == 0:
            results_message = await bot.get_channel(HOTM_VOTING_CHANNEL).send(content="HOTM Voting Results")
        else:
...
async def resethotm(interaction: discord.Interaction):
    ...
    msgs = [msg for msg in await bot.get_channel(HOTM_VOTING_CHANNEL).history().flatten() if
            msg.author.id == 861445044790886467 and msg.content == "HOTM Voting Results"]
    await msgs[0].delete()
