...
async def gostudy(interaction: discord.Interaction,
                  mute_time: Optional[str] = discord.SlashOption(
                      name="time", description="how long should the mute last for? (default: 1 hour)", required=False),
    ...
    if user == None:
        ...
        async def proceedCallBack(interaction):
            ...
            embed.add_field(name="Duration",
                            value=f"<t:{unmute_time}:R>", inline=False)
            ...
            mute.insert_one({"_id": timern, "user_id": str(
                user_id), "unmute_time": str(unmute_time), "muted": True})
